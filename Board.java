	...
	public boolean moveCar(int i, int j, int iDragged, int jDragged){
		...
		if(tempt.direction) {
			...
			if(tempt.i1 < iDragged) {
				for (int a = tempt.i2 + 1; a < (iDragged+tempt.size); a++)
			...
			if(tempt.j1 < jDragged) {
				for (int a = tempt.j2 + 1; a < (jDragged+tempt.size); a++) 
		...
		if((tempt.direction && iDragged + tempt.size >= 6) || (!tempt.direction && jDragged + tempt.size >= 6)) {
		...
		System.out.println("Movement is successfull");
		return true;
	}
